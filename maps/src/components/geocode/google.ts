...
export class Google implements IGeocoder {
  options: GoogleOptions = {
    ...
    apiKey:"AIzaSyC7Gb6_id5CdWz1kunQNAEIOz1j1VhJj74",
  ...
  suggest(query: string, cb: GeocodingCallback,autocomplete:any ,context?: any): void {
    const results: GeocodingResult[] = [];
    autocomplete.getPlacePredictions({input:query},(pred,stat)=>{
      if(stat==="OK"){
        pred.forEach((ele) => {
          results.push({name:ele.description,id:ele.reference})
        });
        cb.call(context,results)
