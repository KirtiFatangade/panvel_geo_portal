...
import { IGeocoder, GeocodingResult } from './api.ts';
import './Control.css';
...
export class GeocoderControl extends EventedControl {
  ...
  onAdd(map: L.Map) {
    ...
    const toggleDiv = L.DomUtil.create('div', `${className}-toggle`, container) as HTMLDivElement;
    const form = this._form = L.DomUtil.create('div', `${className}-form`, container) as HTMLDivElement;
    

    this._map = map;
    this._container = container;
    let isToggleDivVisible = true; 
    ...
    const iconSpanToggle = L.DomUtil.create('span', '', toggleDiv) as HTMLSpanElement;
    iconSpanToggle.innerHTML = '<i class="fa-solid fa-magnifying-glass"></i>'; 
    Object.assign(iconSpanToggle.style, {
        fontSize: '18px',
        margin:'0 auto',
        cursor:'pointer'
    });
    ...
    const inputContainer = L.DomUtil.create('div', '', form) as HTMLDivElement;
    ...
    const iconSpanInput = L.DomUtil.create('span', '', inputContainer) as HTMLSpanElement;
    iconSpanInput.innerHTML = '&#128269;'; 
    Object.assign(iconSpanInput.style, {
        fontSize: '18px',
        margin: '6px',
        color:'white',
    });
    ...
    const input = this._input = L.DomUtil.create('input', '', inputContainer) as HTMLInputElement;
    input.type = 'text';
    ...
    const resultsContainer = L.DomUtil.create('div', `${className}-results`, form) as HTMLDivElement;
    ...
    const removeButton = L.DomUtil.create('span', className + '-remove', inputContainer) as HTMLSpanElement;
    ...
    L.DomEvent.addListener(removeButton, 'click', () => {
      if (isToggleDivVisible) {
        toggleDiv.style.display = 'none';
        toggleDiv.style.width = 'auto'; 
        inputContainer.style.display = 'flex';
        input.focus();
        isToggleDivVisible = false;
    } else {
        toggleDiv.style.display = 'flex';
        toggleDiv.style.width = '47px'; 
        
        inputContainer.style.display = 'none';
        isToggleDivVisible = true;
    }
    });

// const removeButton = L.DomUtil.create('span', className + '-remove', container) as HTMLSpanElement;
    // removeButton.innerHTML = '<i class="fa-solid fa-xmark"></i>';
    // removeButton.style.cursor = "pointer";
    ...
    // iconSpanInput.addEventListener('click', () => {
    //     toggleDiv.style.display = 'flex';
    //     toggleDiv.style.width = '47px'; 
    //     inputContainer.style.display = 'none';
    //     isToggleDivVisible = true;
    ...
    this.search = new google.maps.places.PlacesService(map);
    ...
    this._alts.style.width = "200px";
    this._alts.style.backgroundColor = "black";
  ...
  private _geocodeResult(results: GeocodingResult[], suggest: boolean) {
    console.log("Geocoding Results:", this._results);
  ...
  private _geocode(suggest?: boolean) {
    ...
    this.options.geocoder.suggest(value, cb, this.autocomplete);
  ...
  private _geocodeResultSelected(geocode: GeocodingResult) {
    ...
    if (result.id) {
      this.search.getDetails({ "placeId": result.id, fields: ["geometry"] }, (place, status) => {
    ...
    switch (e.keyCode) {
      ...
      case 13:
        if (this._selection) {
          ...
          this._geocodeResultSelected(this._results[0]);
        }
        break;
      default:
        return;
