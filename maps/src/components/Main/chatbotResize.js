...
export default function Chatbot({ id, setShowChatbot }) {
  ...
  const { userInfo, getCsrfToken, lastRect,
    drawnItems, map, layerControls, Fstep, setFstep, CsDate, setCSdate, selTab, chatSmart, setSmart, Canvas, mapBox, } = useContext(GlobalContext);
  ...
  const [chatSectionSize, setChatSectionSize] = useState({ width: 504, height: 670 });
  ...
  async function sendMessage(prompt = null) {
    ...
    if (message && message !== "") {
      ...
      await fetch(`${HOST}/chat-history/${userInfo.id}/${previousChatId}/${mode}`, {
        ...
        body: JSON.stringify({ message: message, response: responseText, box: box, extent: extent }),
      })
        .then((response) => response.json())
        .then(data => {
          console.log(data);
  ...
  function Visualize(resp) {
    ...
    resp["data"].forEach(data => {
      ...
      if (data["type"] === "geo") {
        Canvas.addLayerUrl(name, `https://portal.vasundharaa.in/${data["data"]}`);
          ...
          if (window.location.pathname.startsWith("/MapBox") && mapBox) {
            ...
            try {
              ...
              map.on("zoomend", () => {
                ...
                if (zoom <= 5) {
                  toggleCanvasLayers("waterbody_watershed", "waterbody_sub_basin", "waterbody_micro_watershed");
                } else if (zoom <= 8) {
                  toggleCanvasLayers("waterbody_sub_basin", "waterbody_watershed", "waterbody_micro_watershed");
                } else {
                  toggleCanvasLayers("waterbody_micro_watershed", "waterbody_sub_basin", "waterbody_watershed");
                }
              });
              ...
              layer.addTo(map);
              layerControls.addOverlay(layer, data["name"] ? data["name"] : name, false, false, false, false, false);
            } catch (e) {
  ...
  const handleMouseDown = (e) => {
    ...
    const isAtRightEdge = Math.abs(e.clientX - divRect.right) <= 2; // Allow a small buffer to detect the exact edge

    // Only enable resizing if the mouse is exactly at the right edge
    if (isAtRightEdge) {
      setIsResizing(true);
  ...
  }, [isResizing, headerSize.height, chatSectionSize.height, footerSize.height]);

  useEffect(() => {
    const resizeHandle = document.querySelector('.react-resizable-handle-se');
    const footerSection = document.querySelector('.chatbot-footer-section');

    if (resizeHandle && footerSection) {
      footerSection.appendChild(resizeHandle);
    ...
    return () => {
      if (resizeHandle && footerSection) {
        footerSection.removeChild(resizeHandle);
      }
    };
  }, []);
    ...
    <>
      {/* <div style={{ margin: '20px' }}>
            <ResizableBox
                ...
                maxConstraints={[1400, 850]}
            >
                <div style={{ backgroundColor: 'lightblue', height: '100%', padding: '10px' }}>
                Resize Me!
                </div>
            </ResizableBox>
            </div> */}
      ...
      <ResizableBox
        ...
        maxConstraints={[1400, 850]}
      >
        <div className="chatbot-section-container" style={{ cursor: 'grab' }}
        >
          {/* Chatbot header section */}
          <div className="chatbot-header-section" >
            <div>
              <label className='text-white' style={{ fontSize: '15px' }}>Mode : </label>
              <select
                id="smart"
                value={mode}
              ...
              >
                ...
                <option value={3}>Automated Operations</option>

              </select>
            </div>
            <div className="close-chatbot">
              <i className="fa-solid fa-xmark" onClick={() => { setShowChatbot(false) }}></i>
            </div>
          </div>
                ...
                {child.sender === 'user' ? (
                    ...
                    {child.text !== "" ? (
                      <>
                        <div className="p-3 ms-3 position-relative" style={{ borderRadius: "15px", backgroundColor: "rgba(57, 192, 237, 0.2)", minWidth: "120px" }}>
                          <p style={{ marginBottom: '20px', fontSize: '15px' }}>{child.text}</p>
                  ...
                  <>
                    ...
                    <div className="p-3 border position-relative" style={{ borderRadius: "10px", backgroundColor: "#fbfbfb", maxWidth: '75%', overflowX: 'auto', overflowY: 'auto' }}>
                      ...
                      {child.mode && (child.mode === 3) && (
                        child.text !== "Please Change Prompt" ?
                          <p className="small mb-0" style={{ maxWidth: '100%', overflowWrap: 'break-word' }}>
                            <button onClick={() => Visualize(child.text)}>Visualize Layer</button>
                          </p> : (<p className="small mb-0" style={{ maxWidth: '100%', overflowWrap: 'break-word' }}>
                            <Markdown>{child.text}</Markdown>
                          </p>)
                      )}
          ...
          <div className="chatbot-footer-section">
            ...
            <div className="button-group">
              ...
              {mode === 1 && <button title="add file" onClick={handleButtonClick}>
...
//     setDimensions({
//       width: ref.offsetWidth + delta.width,
//       height: ref.offsetHeight + delta.height
//     });
//   };
