...
function NDVIChange(){
    ...
    function CreateNDVI(data) {
        ...
        layerControls.addOverlay(layer, `Green Cover ${SDate}`, false, false, false, false, false, data.geo_1[1]);
        ...
        try {
            ...
            logToServer('error', `Error processing change layers: ${e.message}`);
    ...
    async function NDVI() {
        ...
        try {
            await fetch(`${HOST}/ndvi-change`, {
                ...
                .then((data) =>{
                    CreateNDVI(data);
                    logToServer('info','Fetching NDVI change data')
                } );
        } catch (error) {
            alert("The area you're trying to work with is too large. Please try with a smaller area.");
            logToServer('error', `Error fetching NDVI change data:  ${error.message}`);
        ...
        <div style={{display:"flex",flexDirection:"column", marginTop:"10px"}}>
            ...
            <>
                    <div style={{ marginTop: "15px" }}>
                      ...
                      <input style={{ fontSize: '12px', padding: "3px 10px", borderRadius: "5px" }} type={"date"} id="start" min={ "2017-01"}  onChange={(e) => setSdate(e.target.value)} name="start" defaultValue={SDate} />
                    </div>
                    <div style={{ marginTop: "15px" }}>
                      <label htmlFor="end" style={{ marginRight: "15px", color: "#FAF8D4", fontSize: '12px', display: "inline-block" }}>End Date:</label>
                      <input style={{ fontSize: '12px', padding: "3px 10px", borderRadius: "5px" }} type={ "date"} id="end" name="end" min={ "2017-01"}  defaultValue={Edate} onChange={(e) => setEdate(e.target.value)} />
                    </div>
                  </>
            </>
            <>
