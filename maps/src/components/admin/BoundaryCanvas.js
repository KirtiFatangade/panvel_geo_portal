...
var isRingBbox = function (ring, bbox) {
    if (ring.length !== 4) {
        logToServer('warm',`Ring does not have 4 points:${ring}`)
        return false;
    }
...
var ExtendMethods = {
    _toMercGeometry: function(b, isGeoJSON) {
        logToServer('info','Converting to Mercator geometry')
        var res = [];
        var c, r, p,
            mercComponent,
            mercRing,
    ...
    _getOriginalMercBoundary: function () {
        if (this._mercBoundary) {
            logToServer('debug','Using cached Mercator boundary')
            return this._mercBoundary;
        }
        logToServer('info','Calculating original Mercator boundary')
        var compomentBbox, c;
    ...
    _getClippedGeometry: function(geom, bounds) {
        logToServer('info','Clipping geometry to bounds')
        var clippedGeom = [],
            clippedComponent,
            clippedExternalRing,
            clippedHoleRing,
        ...
        if (clippedGeom.length === 0) { //we are outside of all multipolygon components
            logToServer('debug','Geometry is outside bounds')
            return {isOut: true};
        }
        ...
        for (iC = 0; iC < clippedGeom.length; iC++) {
            if (isRingBbox(clippedGeom[iC][0], bounds)) {
                ...
                if (clippedGeom[iC].length === 1) {
                    logToServer('debug','Geometry is inside bounds')
                    return {isIn: true};
                }
        ...
        logToServer('debug','Geometry is inside all holes')
        return {isOut: true};
    },
    ...
    _getTileGeometry: function (x, y, z, skipIntersectionCheck) {
        if ( !this.options.boundary) {
            logToServer('info','No boundary defined')
            return {isIn: true};
        }
